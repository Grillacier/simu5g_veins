...
bool VeinsInetSampleApplication::startApplication()
{
    ...
    if (getParentModule()->getIndex() == 0) { //if this is the first node
        auto callback = [this]() {
            getParentModule()->getDisplayString().setTagArg("i", 1, "red");

            traciVehicle->setSpeed(0); //stop the car

            auto payload = makeShared<VeinsInetSampleMessage>(); //the car creates a message
            std::string m = "J'ai calé à mon grand âge, la honte.";
            payload->setChunkLength(B(100 + 8 * m.length()));
            payload->setRoadId(traciVehicle->getRoadId().c_str());
            payload->setMess(m.c_str());
